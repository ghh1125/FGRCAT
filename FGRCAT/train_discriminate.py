...
def main():
    ...
    def add_reason_to_train_data(train_data, causal_graph):
        for sample in train_data:
            ...
            if premise in causal_graph:
                successors = list(causal_graph.successors(premise))
                if successors:
                    next_node = successors[0]
                    reason_text = "{Tag}Conceptual Explanation:" + str(next_node)
                    sample['premise'] = premise + reason_text
            else:
                reason_text = "{Tag}Conceptual Explanation:"
                sample['premise'] = premise + reason_text
    ...
    def add_new_data(train_data):
        ...
        for sample in train_data_copy:
            ...
            tag_index = premise.find('{Tag}')  # 查找 {Tag} 的位置
            if tag_index != -1:
                text_before_tag = premise[:tag_index]  # 截取 {Tag} 前面的内容
                text_after_tag = premise[tag_index:]  # 截取 {Tag} 后面的内容
                words_before_tag = text_before_tag.split()  # 将 {Tag} 前面的内容按空格分隔成单词列表
                if words_before_tag:
                    random_word_index = random.randint(0, len(words_before_tag) - 1)  # 随机选择一个单词索引
                    word_to_replace = words_before_tag[random_word_index]  # 获取要替换的单词
                    perturbed_word = replace_synonyms(word_to_replace)  # 替换单词的同义词
                    words_before_tag[random_word_index] = perturbed_word  # 将替换后的单词放回列表中
                    perturbed_text = ' '.join(words_before_tag)  # 将替换后的单词列表重新组合成文本
                    perturbed_text += text_after_tag  # 将 {Tag} 后面的内容添加到替换后的文本末尾
    ...
    causal_graph = load_causal_graph('/home/amax/ghh/e-CARE-main/dataset/CIrelated.json')

    train_reversed = train_data.copy()
    train_reversed1 = []
    count = 0
    for sample in train_reversed:
        count = count + 1
        print(str(count) + "/" + "14928")
        premise = sample['premise']
        opposite_sentence = generate_opposite_sentence_chatGLM(premise)
        label = sample['label']
        ...
        if label == 1:
            ...
            label = 1
        opposite_sentence_last_sentence = ""
        ...
        sample['premise'] = opposite_sentence_last_sentence
        sample['label'] = label
        print("NewLabel = ", sample['label'])
        train_reversed1.append(sample)
    ...
    train_data = train_data + new_data1 + train_reversed1
    print(len(train_data))
            ...
            if step % hps.evaluation_step == 0 and step != 0:
                ...
                with torch.no_grad():
                    ...
                    if dev_accu >= best_accuracy:
                        ...
                        if hps.loss_func == 'CrossEntropy':
                            te_soft_accu, te_exact_accu, te_loss = evaluation(hps, test_dataloader, model,
                                                                              loss_function)
                            print('\n')
