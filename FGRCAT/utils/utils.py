...
def evaluation(hps, dataloader, model, loss_function, mode='train'):
    ...
    for batch in dataloader:
        ...
        for i in range(len(predictions)):
            # 根据预测值判断概率值
            if predictions[i] >= 0:
                probability = predictions[i] / (predictions[i] + 1)  # 将预测值映射到 (0, 1) 区间
            else:
                probability = 1 / (1 - predictions[i])  # 将预测值映射到 (0, 1) 区间
            ...
            # 将结果字典添加到列表中
            resultspredictions.append(result_dict)

        file_path = os.path.join(output_dir, 'finally2024.json')
        with open(file_path, 'w') as f:
            json.dump(resultspredictions, f, indent=1)
    ...
    file_path = os.path.join(output_dir, '20240415.json')
    with open(file_path, 'w') as f:
