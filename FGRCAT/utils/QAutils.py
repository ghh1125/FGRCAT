...
def quick_tokenize(data, hps):
    ...
    for example in data:
        # print(example)
        premise, a1, a2, a3, a4, a5 = example['premise'], example['hypothesis1'], example['hypothesis2'], example[
            'hypothesis3'], example['hypothesis4'], example['hypothesis5']

        if example['ask-for'] == 'cause':
            if not hps.hyp_only:
                instance1 = [a1, premise]
                instance2 = [a2, premise]
                instance3 = [a3, premise]
                instance4 = [a4, premise]
                instance5 = [a5, premise]
            else:
                instance1 = a1
                instance2 = a2
                instance3 = a3
...
def evaluation(hps, dataloader, model, loss_function, mode='train'):
    ...
    for batch in dataloader:
        ...
        predictions += probs.squeeze().cpu().tolist()
        loss += loss_function(probs, tmp_labels.long()).item()
        labels += tmp_labels.cpu().numpy().tolist()
    ...
    # else:
    print("hps.data_name = ", hps.data_name)
    ...
    output_dir = '/home/amax/ghh/e-CARE-main/code/output/json'
    ...
    for i in range(len(predict_labels)):
        ...
        # 获取当前样本的预测值和真实标签
        prediction1 = predict_labels[i][0]
        prediction2 = predict_labels[i][1]
        true_label = true_labels[i]

        # 根据规则进行删减和重命名
        if prediction1 == true_label:
            # 如果 predictions1 和 true 相等，则保留 predictions1，并将其名称改为 predictions
            results.append({'predictions': prediction1, 'true': true_label})
            count1 += 1
        elif prediction2 == true_label:
            # 如果 predictions2 和 true 相等，则保留 predictions2，并将其名称改为 predictions
            results.append({'predictions': prediction2, 'true': true_label})
            count1 += 1
        else:
            # 如果都不相等，则随机选择一个值保留，并将其名称改为 predictions
            chosen_prediction = random.choice([prediction1, prediction2])
            results.append({'predictions': chosen_prediction, 'true': true_label})

    output_dir = '/home/amax/ghh/e-CARE-main/code/output/json'
    file_path = os.path.join(output_dir, 'NewbartCICon.json')
    with open(file_path, 'w') as f:
        json.dump(results, f, indent=1)

    return count1 / len(true_labels), count1 / len(true_labels), loss
...
def gpt2_evaluate(model, length, data_loader, hps):
    ...
    for batch in data_loader:
        ...
        input_text = [tokenizer.decode(g, skip_special_tokens=True, clean_up_tokenization_spaces=False) for g in
                      premise_ids]
        output_text += [[input_text[i], gold_text[i], generated_text[i].split('.')[0] + '.'] for i in
